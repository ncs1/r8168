...
static const struct {
	...
	_R("RTL8168FP/8111FP", CFG_METHOD_34,
	   RxCfg_128_int_en | RxEarly_off_V2 | Rx_Single_fetch_V2 |
		   (RX_DMA_BURST << RxCfgDMAShift),
	   0xff7e5880, Jumbo_Frame_9k),

	_R("RTL8168H/8111H", CFG_METHOD_35,
	   RxCfg_128_int_en | RxEarly_off_V2 | Rx_Single_fetch_V2 |
		   (RX_DMA_BURST << RxCfgDMAShift),
	   0xff7e5880, Jumbo_Frame_9k),
				    ...
				    int count, int *eof, void *data)
{
	...
	len += snprintf(
		...
		rtl_chip_info[tp->chipset].name, dev->mtu, tp->num_rx_desc,
		tp->cur_rx, tp->dirty_rx, tp->num_tx_desc, tp->cur_tx,
		tp->dirty_tx, tp->rx_buf_sz, tp->esd_flag, tp->pci_cfg_is_read,
...
static void rtl8168_clear_mcu_ocp_bit(struct rtl8168_private *tp, u16 addr,
				      u16 mask)
{
	rtl8168_clear_and_set_mcu_ocp_bit(tp, addr, mask, 0);
}
...
static u8 rtl8168_check_ephy_addr(struct rtl8168_private *tp, int addr)
{
	...
	if (addr & (BIT_6 | BIT_5))
		rtl8168_clear_and_set_mcu_ocp_bit(tp, 0xDE28, (BIT_1 | BIT_0),
						  (addr >> 5) &
							  (BIT_1 | BIT_0));
...
static void _rtl8168_ephy_write(struct rtl8168_private *tp, int addr, int value)
{
	...
	RTL_W32(tp, EPHYAR,
		EPHYAR_Write | (addr & EPHYAR_Reg_Mask) << EPHYAR_Reg_shift |
...
static bool rtl8168_test_phy_ocp_v2(struct rtl8168_private *tp)
{
...
static bool rtl8168_test_phy_ocp_v3(struct rtl8168_private *tp)
{
	bool RestorePhyOcpReg = FALSE;
	...
	if (uc_response)
		PhyRegValue |= BIT_13;
	if (nctl_pc_range_fail)
		PhyRegValue |= BIT_12;
	if (nctl_pc_stuck_fail)
		PhyRegValue |= BIT_11;
	ClearAndSetEthPhyBit(tp, 0x14, 0xF800, PhyRegValue);
	rtl8168_mdio_write(tp, 0x1F, 0x0000);
...
static bool rtl8168_check_dash_other_fun_present(struct rtl8168_private *tp)
{
...
static void rtl8168_set_mac_mcu_8168h_2(struct net_device *dev)
{
	struct rtl8168_private *tp = netdev_priv(dev);
	u16 i;
	static const u16 mcu_patch_code_8168h_1[] = {
		0xE008, 0xE00F, 0xE011, 0xE047, 0xE049, 0xE073, 0xE075, 0xE07A,
		...
		0xC602, 0xBE00, 0x0CA4, 0x48C1, 0x48C2, 0x9C46, 0xC402, 0xBC00,
		0x0578, 0xC602, 0xBE00, 0x0000
	};
...
static void rtl8168_set_mac_mcu_8168h_3(struct net_device *dev)
{
	struct rtl8168_private *tp = netdev_priv(dev);
	u16 i;
	static const u16 mcu_patch_code_8168h_3[] = {
		0xE008, 0xE00A, 0xE00C, 0xE00E, 0xE010, 0xE039, 0xE03B, 0xE064,
		0xC602, 0xBE00, 0x0000, 0xC602, 0xBE00, 0x0000, 0xC602, 0xBE00,
		0x0000, 0xC602, 0xBE00, 0x0000, 0xC727, 0x76E2, 0x49EE, 0xF1FD,
		0x1E00, 0x8EE0, 0x1E1C, 0x8EE2, 0x76E2, 0x49EE, 0xF1FE, 0xC61D,
		0x8EE0, 0x1E1D, 0x486F, 0x8EE2, 0x76E2, 0x49EE, 0xF12C, 0xC716,
		0x76E0, 0x48E8, 0x48E9, 0x48EA, 0x48EB, 0x48EC, 0x9EE0, 0xC709,
		0xC609, 0x9EF4, 0xC608, 0x9EF6, 0xB007, 0xC602, 0xBE00, 0x0ACC,
		0xE000, 0x03BF, 0x07FF, 0xDE24, 0x3200, 0xE096, 0xC602, 0xBE00,
		0x0000, 0x8EE6, 0xC726, 0x76E2, 0x49EE, 0xF1FD, 0x1E00, 0x8EE0,
		0x1E1C, 0x8EE2, 0x76E2, 0x49EE, 0xF1FE, 0xC61C, 0x8EE0, 0x1E1D,
		0x486F, 0x8EE2, 0x76E2, 0x49EE, 0xF1FE, 0xC715, 0x76E0, 0x48E8,
		0x48E9, 0x48EA, 0x48EB, 0x48EC, 0x9EE0, 0xC708, 0xC608, 0x9EF4,
		0xC607, 0x9EF6, 0xC602, 0xBE00, 0x0ABE, 0xE000, 0x03BF, 0x07FF,
		0xDE24, 0x3200, 0xE096, 0xC602, 0xBE00, 0x0000, 0x0000, 0x0000,
...
static void rtl8168_set_mac_mcu_8168fp_8116as_2(struct net_device *dev)
{
	struct rtl8168_private *tp = netdev_priv(dev);
	u16 i;
	static const u16 mcu_patch_code_8168fp_8116as_2[] = {
		0xE008, 0xE00A, 0xE00F, 0xE014, 0xE016, 0xE018, 0xE01A, 0xE01C,
		0xC602, 0xBE00, 0x2AB2, 0x1BC0, 0x46EB, 0x1BFE, 0xC102, 0xB900,
		0x0B1A, 0x1BC0, 0x46EB, 0x1B7E, 0xC102, 0xB900, 0x0BEA, 0xC602,
		0xBE00, 0x0000, 0xC602, 0xBE00, 0x0000, 0xC602, 0xBE00, 0x0000,
		0xC602, 0xBE00, 0x0000, 0xC602, 0xBE00, 0x0000
	};
	...
	for (i = 0; i < ARRAY_SIZE(mcu_patch_code_8168fp_8116as_2); i++) {
		rtl8168_mac_ocp_write(tp, 0xF800 + i * 2,
				      mcu_patch_code_8168fp_8116as_2[i]);
	}
...
static void _rtl8168_set_mac_mcu_8168fp_2(struct net_device *dev)
{
	struct rtl8168_private *tp = netdev_priv(dev);
	u16 i;
	static const u16 mcu_patch_code_8168fp_2[] = {
		0xE008, 0xE00A, 0xE00F, 0xE014, 0xE05F, 0xE064, 0xE066, 0xE068,
		0xC602, 0xBE00, 0x0000, 0x1BC0, 0x46EB, 0x1BFE, 0xC102, 0xB900,
		...
		0x8002, 0x8004, 0x8007, 0x48C1, 0x48C2, 0x9C46, 0xC402, 0xBC00,
...
static void rtl8168_set_mac_mcu_8168fp_3(struct net_device *dev)
{
	...
	static const u16 mcu_patch_code_8168fp_3[] = {
		0xE008, 0xE053, 0xE058, 0xE05A, 0xE05C, 0xE05E, 0xE060, 0xE062,
		...
		0xF000, 0x8002, 0x8004, 0x8007, 0x48C1, 0x48C2, 0x9C46, 0xC402,
		0xBC00, 0x07BC, 0xC602, 0xBE00, 0x0000, 0xC602, 0xBE00, 0x0000,
		0xC602, 0xBE00, 0x0000, 0xC602, 0xBE00, 0x0000, 0xC602, 0xBE00,
...
static void rtl8168_set_phy_mcu_8168h_3(struct net_device *dev)
{
	struct rtl8168_private *tp = netdev_priv(dev);
	unsigned int gphy_val;
...
static void rtl8168_hw_phy_config(struct net_device *dev)
{
	...
	if (tp->mcfg == CFG_METHOD_1) {
		...
		ClearAndSetEthPhyBit(tp, 0x14, 0xFF00, 0x6600);
		rtl8168_mdio_write(tp, 0x13, 0x81CB);
		ClearAndSetEthPhyBit(tp, 0x14, 0xFF00, 0x3500);
		rtl8168_mdio_write(tp, 0x1F, 0x0000);

		rtl8168_mdio_write(tp, 0x1F, 0x0A80);
		ClearAndSetEthPhyBit(tp, 0x16, 0x000F, 0x0005);
		rtl8168_mdio_write(tp, 0x1F, 0x0A43);
		rtl8168_mdio_write(tp, 0x13, 0x8016);
		...
		ClearAndSetEthPhyBit(tp, 0x14, 0xFF00, 0x7300);
		...
		ClearAndSetEthPhyBit(tp, 0x14, 0xF800, 0xB800);
		rtl8168_mdio_write(tp, 0x13, 0x8105);
		ClearAndSetEthPhyBit(tp, 0x14, 0xFF00, 0x0A00);
		...
		ClearAndSetEthPhyBit(tp, 0x14, 0x7000, 0x7000);

		rtl8168_mdio_write(tp, 0x1F, 0x0A43);
		rtl8168_mdio_write(tp, 0x13, 0x8111);
		ClearAndSetEthPhyBit(tp, 0x14, 0xFF00, 0x7C00);
		rtl8168_mdio_write(tp, 0x1F, 0x0000);
...
static void rtl8168_init_software_variable(struct net_device *dev)
{
	...
	if (tp->HwSuppEsdVer == 2 || tp->HwSuppEsdVer == 3) {
		rtl8168_mdio_write(tp, 0x1F, 0x0A46);
...
static void rtl8168_hw_address_set(struct net_device *dev,
...
static int rtl8168_get_mac_address(struct net_device *dev)
{
	...
	if (tp->mcfg == CFG_METHOD_18 || tp->mcfg == CFG_METHOD_19 ||
	    ...
	    tp->mcfg == CFG_METHOD_32 || tp->mcfg == CFG_METHOD_33 ||
	    tp->mcfg == CFG_METHOD_34 || tp->mcfg == CFG_METHOD_35) {
		*(u32 *)&mac_addr[0] =
...
static int rtl8168_enable_msix(struct rtl8168_private *tp)
{
	...
	nvecs = pci_enable_msix_range(tp->pci_dev, msix_ent, 1, 1);
	if (nvecs < 0)
		goto out;
...
static int rtl8168_open(struct net_device *dev)
{
	...
	tp->TxDescArray = dma_alloc_coherent(&pdev->dev, tp->TxDescAllocSize,
	...
	retval = request_irq(tp->irq, rtl8168_interrupt,
			     (tp->features &
			      (RTL_FEATURE_MSI | RTL_FEATURE_MSIX)) ?
				     0 :
				     SA_SHIRQ,
			     dev->name, dev);
	if (retval < 0)
...
err_free_all_allocated_mem:
	if (tp->RxDescArray != NULL) {
		dma_free_coherent(&pdev->dev, tp->RxDescAllocSize,
				  tp->RxDescArray, tp->RxPhyAddr);
		tp->RxDescArray = NULL;
	}

	if (tp->TxDescArray != NULL) {
		dma_free_coherent(&pdev->dev, tp->TxDescAllocSize,
...
static u32 rtl8168_get_txd_opts1(struct rtl8168_private *tp, u32 opts1, u32 len,
				 unsigned int entry)
{
				...
				struct rtl8168_private *tp, napi_budget budget)
{
	...
	/*
         ...
         */
	if ((tp->dirty_rx + tp->num_rx_desc == tp->cur_rx) &&
	    netif_msg_intr(tp))
